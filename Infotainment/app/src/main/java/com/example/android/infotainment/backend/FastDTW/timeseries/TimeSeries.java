...
package com.example.android.infotainment.backend.FastDTW.timeseries;

import android.util.Log;

import com.example.android.infotainment.backend.FastDTW.util.Arrays;
...
public class TimeSeries {
	...
	private final String errorTag = "TS-DTW";;
	...
	public TimeSeries(ArrayList<Object> data) {
		...
		for (Object i : data) {
			try {
				...
				System.out.println("Object can't cast to double: " + e);
	...
	/**
	 * Custom constructor made by the Capstone Project team
	 * @param data: Double array of data points.
	 */
	public TimeSeries(double[] data){
		...
		labels.add("Time");
		for (double i : data){
			tsp = new TimeSeriesPoint(new double[] {i});
			timeReadings.add(new Double(timeReadings.size()));
			tsArray.add(tsp);
		}
	}
	...
	/**
	 * Custom constructor made by the Capstone Project team
	 * @param data: Integer array of data points.
	 */
	...
	public void addLast(double time, TimeSeriesPoint values) {
		...
		if ((this.size() > 0) && (time <= ((Double) timeReadings.get(timeReadings.size() - 1)).doubleValue()))
			throw new InternalError("ERROR:  The point being inserted at the " + "end of the time series does not have "
